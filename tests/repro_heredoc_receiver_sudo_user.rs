...
//!
//! `.agent-config-7vu4q` landed mid-flight and routes here-strings through the
//! same `heredoc_language`, hence the two `<<<` rows: the claim under test is
//! that the two forms share this reader.
//!
...
// ---------------------------------------------------------------------------
// Here-strings share this reader. `.agent-config-7vu4q` landed while this bead
// was in flight and routes a here-string's receiver through `heredoc_language`,
// so every spelling above applies to `<<<` identically. Two rows, because the
// claim is that they share a reader -- not that `<<<` needs its own table.
// ---------------------------------------------------------------------------

#[test]
fn a_sudo_user_argument_does_not_become_a_here_string_receiver() {
    let cmd = format!("{JS_PREFIX}sudo -u root python3 <<< \"{}\"", rmtree());
    assert_denied_by_the_body(&cmd, "python3 receives this here-string, not node");
}

#[test]
fn a_sudo_user_named_node_does_not_make_a_here_string_javascript() {
    let cmd = format!("sudo -u node python3 <<< \"{}\"", rmtree());
