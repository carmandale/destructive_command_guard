...
/// It must still deny content that truly went unjudged
/// (.agent-config-dcg-heredoc-policy-warn-hides-outer-fxck7).
#[test]
fn test_fallback_spares_the_warned_match_and_still_reads_unjudged_content() {
    ...
    // last two extract to text that differs from the raw bytes, so the warned
    // span cannot be mapped back into the command.
    for command in [
    ...
    // Controls: in a body whose spans map back, only the warned text is
    // spared. A python heredoc left unterminated inside the bash body is never
    // read by anything but the fallback, and still denies beside the warned
    // `git reset --hard`.
    let command = "bash <<'EOF'\n\
                   git reset --hard origin/main\n\
                   python3 <<'PY'\n\
    ...
    assert!(
        ...
        "the unread os.remove beside the warned reset must be denied by the fallback\n\
         command: {command}\nstdout: {stdout}\nstderr: {stderr}"
    );

    // A real `rm -rf` handed to os.system -- Medium, so no AST rule stops
    // it -- is still denied by the fallback behind the warned rmtree.
    let command = "python3 <<'PY'\n\
                   import os, shutil\n\
                   shutil.rmtree('/Users/someone/proj/build')\n\
    ...
    assert!(
        stdout.contains("\"permissionDecision\":\"deny\"") && stdout.contains("fallback check"),
        "the rm -rf beside the warned rmtree must be denied by the fallback\n\
         command: {command}\nstdout: {stdout}\nstderr: {stderr}"
    );
