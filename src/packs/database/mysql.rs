    ...
    fn the_gate_does_not_select_this_pack_on_prose() {
        ...
        for cmd in [
            ...
            assert!(
                ...
                "no MySQL client here — the gate must not select this pack: {cmd}"
            );
        }
        ...
        for cmd in [r#"mysql -e "drop table foo""#, r"mysqladmin drop appdb"] {
            assert!(
                pack.check(cmd).is_some(),
                "a real MySQL destructive command must still be denied: {cmd}"
            );
        }
    }
